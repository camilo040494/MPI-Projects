...
//printf("Filas %d hasta %d\n", start, end);

	 int j;
	 for(j=start;j<end;j++, node++){
	   MPI_Send(&m[j],MAXCOL,MPI_INT,node,TAGTAREA,MPI_COMM_WORLD);
	 }
	 ...
	 for(j=start;j<end;j++, node++){
	   int result;
	   MPI_Recv(&result,1,MPI_INT,node,TAGRESULT,MPI_COMM_WORLD, &status);
	   x[j]=result;
	 }
	 i=end;
	 node = 1;
//printf("Vector resultado %d\n", i);

	}

//printf("Termina ejecucion nodo 0");

	for(node=1;node<size;node++){
	 MPI_Send(&i,1,MPI_INT,node,TAGPARAR,MPI_COMM_WORLD);
	}
...
int receive(int rank) {
	...
	*/
	...
	MPI_Status status;
	int b[MAXCOL], v[MAXCOL], i, x=0;
	MPI_Recv(&b,MAXCOL,MPI_INT,0,TAGVECTOR,MPI_COMM_WORLD,&status);

//printf("Recibe vector %d\n", rank);

	MPI_Probe(0,MPI_ANY_TAG,MPI_COMM_WORLD,&status);

	while(status.MPI_TAG!=TAGPARAR){
	  MPI_Recv(&v,MAXCOL,MPI_INT,0,TAGTAREA,MPI_COMM_WORLD,&status);

//printf("Recibe fila %d\n", rank);

	   for(i=0; i<MAXCOL;i++){
	     x+=(b[i]*v[i]);
	   }
	  MPI_Send(&x,1,MPI_INT,0,TAGRESULT,MPI_COMM_WORLD);

//printf("Envia fila %d\n", rank);

	  MPI_Probe(0,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
//printf("Status nodo %d, es: %d\n", rank, status.MPI_TAG);

	}

//printf("Termina nodo %d\n", rank);
